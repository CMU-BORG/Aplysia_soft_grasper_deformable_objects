...
* Support for using a hand-tuned SNS to control the soft grasper gantry system accomplish pick-and-place tasks.

## Code Structure
The main environment for simulating a gantry system with a soft grasper and objects to pick is
in [GantrySimulation.py](Gantry/envs/GantrySimulation.py). The neural network parameters are defined in [SNS_layer.py](Gantry/controller/SNS_layer.py) and the supervised learning environment is defined in [train.py](Gantry/controller/train.py)
